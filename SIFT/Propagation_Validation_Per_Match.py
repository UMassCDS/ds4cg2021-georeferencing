...
if __name__ == '__main__':
    ...
    # with open(f'Data\\PropagationDistancesPerMatch_{datetime.date.today()}.csv', 'w') as f:
    with open('Data\\PropagationDistancesPerMatch_2021-08-12.csv', 'a+') as f:
        # f.write('Source,Target,PtDistance,NumMatches\n')
        mapping = pd.read_csv('Data\\MacConnellMapping.csv')
        done = [line.split(',')[0] for line in open('Data\\PropagationDistancesPerMatch_2021-08-12.csv', 'r').readlines()]
        for c, row in mapping.iterrows():
            REFPATH = row['Center']
            if REFPATH not in done:
                lst = [(row['Tile1'], row['Distance1']), (row['Tile2'], row['Distance2']),
                       (row['Tile3'], row['Distance3']), (row['Tile4'], row['Distance4']),
                       (row['Tile5'], row['Distance5']), (row['Tile6'], row['Distance6']),
